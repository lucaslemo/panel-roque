...
<button {{ $attributes->merge(['type' => 'button', 'class' => 'justify-self-center text-left w-full md:w-96 laptop:w-80 2xl:w-96 rounded-lg border border-primary shadow-picker p-6 md:p-8 ' . $class]) }}>
    <p class="text-normal font-medium mb-2">{{ $customer }}</p>
    <p class="text-small md:text-normal font-normal">{{ __('CNPJ/CPF') }}: {{ $code }}</p>
</button>
