            ...
            @foreach ($invoices as $invoice)
                ...
                <div class="hidden laptop:flex space-x-4 items-center bg-background shadow-card rounded-lg p-4 md:p-6">
                    ...
                    <div class="text-base xl:text-lg text-black truncate w-full middle:w-1/3">
                        <span class="font-medium"># {{ $invoice->extConta }}</span>
                        <span class="font-light"> | {{ __('Payments') }} {{ $invoice->numParcela }}/{{ $invoice->numTotalParcela }}</span>
                    </div>

                    <!-- Data da parcela e vencimento -->
                    <div class="text-base xl:text-lg text-black text-nowrap font-medium w-max middle:w-1/2 2xl:w-1/3">
                        <div>{{ __('Payment date') }}: {{ \Carbon\Carbon::parse($invoice->dtParcela)->format('d/m/Y') }}</div>
                        <div class="font-normal leading-none mt-1">
                    ...
                    <div class="text-base xl:text-lg font-medium text-black w-max middle:w-1/6 2xl:w-1/5 text-nowrap">
                        {{ 'R$ ' . number_format($invoice->vrBruto, 2, ',', '.') }}
                    </div>
                    ...
                    @else
                        <div class="flex justify-end items-center relative w-max invisible">
                            <button class="flex h-12 min-w-20 p-2"></button>
                            <button class="flex h-12 min-w-20 p-2"></button>
                        </div>
                    @endif
                </div>
                ...
                <div class="block laptop:hidden bg-background shadow-card rounded-lg p-4">
                    ...
                    <div class="flex justify-between text-small md:text-base text-black mb-4">
                        <div>
                            <div class="font-medium"># {{ $invoice->extConta }}</div>
                        ...
                        <div class="text-small md:text-base font-medium text-black mb-4">
                            <div>{{ __('Payment date') }}: {{ \Carbon\Carbon::parse($invoice->dtParcela)->format('d/m/Y') }}</div>
                            <div class="font-normal leading-none text-end mt-1">
                                {{ __('Due date') }}: {{ \Carbon\Carbon::parse($invoice->dtVencimento)->format('d/m/Y') }}
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Valor -->
                    <div class="text-small md:text-base font-medium text-black">
                        {{ 'R$ ' . number_format($invoice->vrBruto, 2, ',', '.') }}
                    </div>
